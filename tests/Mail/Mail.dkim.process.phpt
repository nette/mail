...
$signer = new class ('', '', '', null, ['From', 'To', 'Subject', 'X-Mailer', 'Content-Type']) extends DkimSigner {
	public function computeSignature(string $rawHeader, string $signature): string
	{
		...
		Assert::match(
			'DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; s=; t=%i%; c=relaxed/simple; h=From:To:Subject:X-Mailer:Content-Type; d=; bh=ajG6YIACaHQVmGzBb/7kmuYS2aRqla4IYr5sTMwVP7k=; b=',
			$headers,
		);
